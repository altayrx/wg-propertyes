...
class CWG
{
	...
	public static $pagefrompages = '';
	const CREDIT_FROM = 3000;
	const CREDIT_TO = 300000;
	const PRODUCTS_IBLOCK_ID = 17;

	public static function subdomen()
	{
		return 'www';
		return preg_replace('/\..*/', '', $_SERVER['HTTP_HOST']);
	}

	public static function getCity($filter = Array(), $select = Array())
	{
		if (!\Bitrix\Main\Loader::includeModule('iblock')) {
			ShowError('Модуль iblock не установлен');
			return;
		}
		$filter['IBLOCK_ID'] = self::PRODUCTS_IBLOCK_ID;
		$filter['ACTIVE'] = 'Y';
		$select[] = 'IBLOCK_ID';
		$geoRes = CIBlockElement::getList(
			Array(),
			$filter,
			false,
			false,
		...
		if ($geoRes->selectedRowsCount() > 0) {
			if ($geoOb = $geoRes->fetch()) {
				$geoOb['C'] = $geoRes->selectedRowsCount();
				return $geoOb;
			}
		}
		return;
	...
	public static function getCityById($id)
	{
		...
		$geoRes = CIBlockElement::getList(
			Array(),
			Array('IBLOCK_ID' => PRODUCTS_IBLOCK_ID, 'ID' => $id, 'ACTIVE' => 'Y'),
			false,
			false,
	...
	public static function getCityByCode($code)
	{
		...
		$geoRes = CIBlockElement::getList(
			Array(),
			Array('IBLOCK_ID' => self::PRODUCTS_IBLOCK_ID, 'CODE' => $code),
			false,
			false,
	...
	public static $geocityCache = Array();

	public static function geocity()
	{
		if (!empty($geocityCache)) {
			return $geocityCache;
		} else {
			if (!\Bitrix\Main\Loader::includeModule('iblock')) {
				ShowError('Модуль iblock не установлен');
			...
			$geoRes = CIBlockElement::getList(
				Array(),
				Array('IBLOCK_ID' => self::PRODUCTS_IBLOCK_ID, 'CODE' => $sub, 'ACTIVE' => 'Y'),
				false,
				false,
				Array('IBLOCK_ID', 'ID', 'NAME', 'PROPERTY_PREPOSITIONAL', 'PROPERTY_DATIVE', 'PROPERTY_ADDRESS', 'PROPERTY_PHONE')
			);
			if ($geoRes->selectedRowsCount() > 0) {
				if ($geoOb = $geoRes->fetch()) {
					$geoOut = Array(
					...
					if ($geoOb['PROPERTY_PREPOSITIONAL_VALUE']) {
						$geoOut['prepositional'] = $geoOb['PROPERTY_PREPOSITIONAL_VALUE'];
					}
					if ($geoOb['PROPERTY_DATIVE_VALUE']) {
						$geoOut['dative'] = $geoOb['PROPERTY_DATIVE_VALUE'];
					}
					if ($geoOb['PROPERTY_ADDRESS_VALUE']) {
						$geoOut['address'] = $geoOb['PROPERTY_ADDRESS_VALUE'];
					}
					if ($geoOb['PROPERTY_PHONE_VALUE']) {
						$geoOut['phone'] = $geoOb['PROPERTY_PHONE_VALUE'];
					}
					$geocityCache = $geoOut;
					return $geoOut;
				}
	...
	{
		mail('vadim@interno.ru', 'mlog', json_encode($data));
	}

	public static function wgUrlEncode($str)
	{
		return preg_replace("/ /", "_", $str);
	}

	public static function requestFilter($str) {
		$opts = array('http' =>
			array(
				'method'  => 'POST',
				'content' => 'decrypt=1&str=' . $str
			)
		);
		$context  = stream_context_create($opts);
		$url = 'https://www.whitegoods.ru/local/ajax/crypt.php';
		//$url = 'http://host45/local/ajax/crypt.php';
		//$url = 'https://24.interno.ru/local/ajax/crypt.php';
		$preresult = explode("&", file_get_contents($url, false, $context));
		$result = Array();
		foreach ($preresult as $pre) {
			$tmp = explode('=', $pre);
			$result[] = Array($tmp[0], explode('#', $tmp[1]));
		}
		return $result;
	}

	public static function getElementIdByCode($code)
	{
		global $DB;
		$sql ='
...
ORDER BY card_order;';
	...
	public static function addSectionExtraFields($sectionId, $extraFieldsName, $extraFieldsSet, $extraFieldsRange, $options)
	{
		...
		file_put_contents($_SERVER['DOCUMENT_ROOT'] . '/logs/a1.log', "update: " . print_r($update, 1) . PHP_EOL, FILE_APPEND);
		file_put_contents($_SERVER['DOCUMENT_ROOT'] . '/logs/a1.log', "update: " . print_r($options, 1) . PHP_EOL, FILE_APPEND);
	...
	{
		file_put_contents($_SERVER['DOCUMENT_ROOT'] . '/logs/a1.log', "change" . PHP_EOL . print_r([$sectionIdOld, $sectionIdNew], 1) .PHP_EOL, FILE_APPEND);
		global $DB;
		$DB->PrepareFields("extra_fields");
		$DB->StartTransaction();
		$DB->Update(
...
';
				...
				/*if ($sectionId == 1044) {
					file_put_contents($_SERVER['DOCUMENT_ROOT'] . '/filter4.log', print_r($ob, 1) . PHP_EOL, FILE_APPEND);
				}*/
			}
		}
		return $filter;
...
//file_put_contents($_SERVER['DOCUMENT_ROOT'] . '/logs/ep.log', print_r($extraValue, 1) . PHP_EOL . print_r($DB->forSql($extraValue), 1) . PHP_EOL . '* * *' . PHP_EOL, FILE_APPEND);
	...
	{
		global $DB;
//$product['products_extra_fields_value'] = $DB->forSql($product['products_extra_fields_value']);
//file_put_contents($_SERVER['DOCUMENT_ROOT'] . '/logs/ep3.log', print_r($product, 1) . PHP_EOL . print_r($DB->forSql($product), 1) . PHP_EOL . '* * *' . PHP_EOL, FILE_APPEND);
		$sql = '
SELECT products_id
FROM products_to_extra_fields
WHERE
...
/*start selections*/
	public function selectionsInit()
	{
		file_put_contents($_SERVER['DOCUMENT_ROOT'] .  '/logs/si.log', print_r($_GET, 1));
		if (
			...
			return array(
				"TABSET" => "WGSELECTIONS",
				"GetTabs" => array("CWG", "GetSelectionsTabs"),
				"ShowTab" => array("CWG", "ShowSelectionsTab"),
				"Action" => array("CWG", "SelectionsAction"),
				"Check" => array("CWG", "SelectionsCheck")
			);
		}
	}

	public function GetSelectionsTabs($arArgs)
	{
		// SORT - после какого стандартного таба вставлять. Не установлено - после последнего
		$arTabs = array(
			array("DIV" => "wgselectionsedit", "TAB" => "Подборки", "ICON" => "sale", "TITLE" => "Подборки", 'SORT' => 2),
		);
		return $arTabs;
	}
	public function SelectionsAction($arArgs)
	{
		return true;
	}
	public function SelectionsCheck($arArgs)
	{
		return true;
	}
	public function ShowSelectionsTab($divName, $arArgs, $bVarsFromForm)
	{
		if ($divName == "wgselectionsedit") {
			if (!\Bitrix\Main\Loader::includeModule('iblock')) {
				ShowError('Модуль iblock не установлен');
				return;
			}
			$selectionsRes = CIBlockSection::getList(
				Array(),
				Array('IBLOCK_ID' => self::PRODUCTS_IBLOCK_ID, 'ID' => $arArgs['ID']),
				false,
				Array('ID', 'LEFT_MARGIN', 'RIGHT_MARGIN')
			);
			if ($selectionsOb = $selectionsRes->fetch()) {
				$section_id = $selectionsOb['ID'];
				$section_left = $selectionsOb['LEFT_MARGIN'];
				$section_right = $selectionsOb['RIGHT_MARGIN'];
			}
			$tab_content  = "<tr><td>";
			$tab_content .= "<h2>Свойства для подборок</h2>";
			$tab_content .= "<table>";
			global $DB;
			$sql ='
SELECT * FROM selections
WHERE section_id=' . $section_id . ' AND type="property"';
			$res = $DB->Query($sql, false, __LINE__);
			$props = Array();
			$properties = '';
			while($ob = $res->fetch()) {
				$tab_content .= "<tr class='property-row' data-property='" . $ob['id'] . "'><td><input type='text' size='39' maxlength='255' value='" . $ob['name'] . "' /></td><td><button class='property-save'>Сохранить</button></td><td><button class='property-remove'>Удалить</button></td></tr>";
				$props[$ob['id']] = $ob['name'];
				$properties .= "<label><input name='properties[]' type='checkbox' value='" . $ob['id'] . "'> " . $ob['name'] . "</label>";
			}
			$tab_content .= "<tr><td><input type='text' size='39' maxlength='255' name='propertyadd' /></td><td><button class='property-add'>Добавить</button></td></tr>";
			$tab_content .= "</table>";
			$tab_content .=
"<script>
	var section_id = " . $section_id . ";
	var section_left = " . $section_left . ";
	var section_right = " . $section_right . ";
	var properties = \"<td>" . $properties . "</td>\"
</script>";
			$tab_content .= "<h2>Подборки</h2>";
			$tab_content .= "<table><tr><th>Название</th><th>Сортировка</th><th>Свойства</th><th></th><th></th></tr>";
			global $DB;
			$sql ='
SELECT * FROM selections
WHERE section_id=' . $section_id . ' AND type="selection"
ORDER BY sort ASC';
			$res = $DB->Query($sql, false, __LINE__);
			while($ob = $res->fetch()) {
				$tab_content .= "<tr class='selection-row' data-selection='" . $ob['id'] . "'>";
				$tab_content .= "<td><input type='text' size='39' maxlength='255' value='" . $ob['name'] . "' /><br><small>" . $ob['code'] . "</small></td>";
				$tab_content .= "<td><input type='text' size='15' maxlength='255' value='" . $ob['sort'] . "' /><br><small>&nbsp;</small></td>";
				$tab_content .= "<td>";
				$list = explode('|', $ob['list']);
				foreach ($props as $key => $prop) {
					...
					$tab_content .= "<label><input name='properties[]' type='checkbox'" . $checked . " value='" . $key . "'> " . $props[$key] . "</label>";
				}
				$tab_content .= "</td>";
				$tab_content .= "<td><button class='selection-save'>Сохранить</button></td>";
				$tab_content .= "<td><button class='selection-remove'>Удалить</button></td>";
				$tab_content .= "</tr>";
			}
			$tab_content .= "<tr><td><input type='text' size='39' maxlength='255' name='selectionadd' /></td><td><button class='selection-add'>Добавить</button></td></tr>";
			$tab_content .= "</table>";
			$tab_content .= "</td></tr>";
			echo $tab_content;
		}
...
/*end selections*/
	...
	{
		file_put_contents(
			$_SERVER['DOCUMENT_ROOT'] . '/logs/gcp.log',
			date('Y-m-d H:i:s') . ': ' . $GLOBALS["APPLICATION"]->GetCurPage() . PHP_EOL,
			FILE_APPEND
		);
		...
		if (
			...
			return array(
				"TABSET" => "WGSECTIONPROPS",
				"GetTabs" => array("CWG", "GetSectionTabs"),
				"ShowTab" => array("CWG", "ShowSectionTab"),
				"Action" => array("CWG", "SectionAction"),
				"Check" => array("CWG", "SectionCheck")
			);
		} else {
			//self::mlog($_REQUEST);
		}
	...
	{
		//self::mlog([__LINE__, self::$newProp]);
	...
	{
		file_put_contents($_SERVER['DOCUMENT_ROOT'] . '/logs/a1.log', "check" . PHP_EOL, FILE_APPEND);
		$old = self::getSectionExtraFields(1, $_REQUEST['wgsection']);
		//self::mlog([__LINE__, $old]);
		self::$toDelete = Array();
			...
			if ($newpropname) {
				self::$newProp[$index] = Array(
					...
					Array(
						...
						'is_color' => (isset($_REQUEST['new_wg_is_color'][$index]) ? 1 : 0),
						'is_material' => (isset($_REQUEST['new_wg_is_material'][$index]) ? 1 : 0),
		...
		file_put_contents($_SERVER['DOCUMENT_ROOT'] . '/logs/a1.log', print_r(self::$newProp, 1) . PHP_EOL, FILE_APPEND);
		return true;
	...
	public function GetSectionTabs($arArgs)
	{
		// SORT - после какого стандартного таба вставлять. Не установлено - после последнего
		$arTabs = array(
			array("DIV" => "wgsectionedit", "TAB" => "Свойства раздела", "ICON" => "sale", "TITLE" => "Свойства раздела", "SORT" => 1),
		);
		return $arTabs;
	}
	...
	public function ShowSectionTab($divName, $arArgs, $bVarsFromForm)
	{
		if ($divName == "wgsectionedit") {
			...
			$res = self::getSectionExtraFields(1, $section_code);
			$tab_content = '
<tr class="wg-heading">
	<th rowspan="2">Название</th>
	<th rowspan="2">Значения</th>
	<th rowspan="2">Диапазоны</th>
	<th colspan="4">Карточка</th>
	<th colspan="2">Листинг</th>
	<th colspan="2">Подбор</th>
</tr>
<tr class="wg-heading">
	<th>S</th><th>D</th><th>C</th><th>M</th><th>S</th><th>D</th><th>S</th><th>D</th>
</tr>';
			while ($ob = $res->GetNext()) {
				$select = '';
...
<tr class="adm-detail-file-row adm-wg-section-row">
	...
	{
		/*file_put_contents(
			$_SERVER['DOCUMENT_ROOT'] . '/ea2.log',
			date('y-m-d H:i:s') . ': args=' . print_r($arArgs, 1) . PHP_EOL . '; get_action: ' . $_GET['action'] . PHP_EOL,
			FILE_APPEND
		);*/
		// Основные данные сохранены. Делаем тут действия. 
		// Возвращаем True в случае успеха и False - в случае ошибки
...
<tr style="display:none">
		...
		<input type="hidden" name="wg_section_code" value="'.$section_code.'" />
			...
			if (isset($_GET['action']) && $_GET['action'] == 'copy') {
				...
				window.onload = function() {
					...
					//console.log(['copy', wgprops.length]);
	...
	private static $basket = Array();
	private static $basketEx = Array();

	public static function setBasket($products)
	{
		if (!empty($products)) {
	...
	public static function setBasketEx($products)
	{
		if (!empty($products)) {
			self::$basketEx = $products;
		}
	}
	...
	public static function getBasketEx($product = 0)
	{
		if ($product == 0) {
			return self::$basketEx;
		} else {
			return self::$basketEx[$product];
		}
	}
}
